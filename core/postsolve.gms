...
*** -----B2: if there was a flip-floping of c_peakBudgYr in the previous iterations, try to overome this by adjusting the CO2 price path after the peaking year	
  ...
  if(cm_targetNetNegEmi ge 0, 
    pm_taxCDR(ttot, regi)$(ttot.val le c_peakBudgYr) = pm_taxCO2eq(ttot,regi)$(ttot.val le c_peakBudgYr);
	  pm_taxCDR(t,regi)$(t.val gt c_peakBudgYr) = sum(t2$(t2.val eq 2100),pm_taxCDR(t2,regi));  !! keep CDR subsidy constant after peak year
    );
...
!! adjustment of post peak CDR subsidy to regulate the amount of net negative emissions if specified in cm_targetNetNegEmi 
if(cm_emiscen eq 9 AND cm_targetNetNegEmi ge 0,
  !! calculate cumulative net negative emissions after peak year (as positive number)
  s_actualNetNegEmi = - sum((regi,ttot)$(c_peakBudgYr le ttot.val AND ttot.val le 2100 ),
    vm_emiAll.l(ttot,regi,"co2")
    * ( !! second half of the peak year period: e.g. 2050-2052 = 3 years if c_peakBudgYr is 2050)
        (pm_ts(ttot) / 2 + 0.5)$( ttot.val eq c_peakBudgYr )
        !! entire middle periods
